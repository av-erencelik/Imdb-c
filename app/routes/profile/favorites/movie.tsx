...
export async function loader({ request }: { request: Request }) {
  try {
    ...
    https://api.themoviedb.org/3/account/a/favorite/movies?api_key=${process.env.API_KEY}&session_id=${sessionId}&sort_by=created_at.desc&page=1`);

    const favorites = await favoritesResponse.json();
    if (favorites.status_code) {
      throw "error";
    }
    return { favorites };
  } catch (e) {
  ...
  return (
    <Container maxW="container.xl" py="3">
      ...
      <Flex flexDirection="column" gap="3">
        {favorites.results.map((result: ResultMovie) => {
          return (
            <Card
              key={result.id}
              overflow="hidden"

...
export async function loader({ request }: { request: Request }) {
  try {
    ...
    const favoritesResponse = await fetch(`
        https://api.themoviedb.org/3/account/a/favorite/tv?api_key=${process.env.API_KEY}&session_id=${sessionId}&sort_by=created_at.desc&page=1`);
  ...
  return (
    <Container maxW="container.xl" py="3">
      ...
      <Flex flexDirection="column" gap="3">
        {favorites.results.map((result: ResultTvShow) => {
          return (
            <Card
              key={result.id}
              overflow="hidden"

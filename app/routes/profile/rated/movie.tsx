...
export async function loader({ request }: { request: Request }) {
  try {
    ...
    const ratedResponse = await fetch(`
        https://api.themoviedb.org/3/account/a/rated/movies?api_key=${process.env.API_KEY}&session_id=${sessionId}&sort_by=created_at.desc&page=1`);
    const rated = await ratedResponse.json();
    if (rated.status_code) {
      throw json("error");
    }
    return { rated };
  } catch (e) {
...
export async function action({ request }: LoaderArgs) {
  ...
  const response = await fetch(
    `https://api.themoviedb.org/3/movie/${formData.get("id")}/rating?api_key=${
      process.env.API_KEY
  ...
  return (
    <Container maxW="container.xl" py="3">
      ...
      <Flex flexDirection="column" gap="3">
        {rated.results.map((result: ResultMovie) => {
          return (
            <Card
              key={result.id}
              overflow="hidden"
